...
const toDomainWidget = (dbWidget: RawWidget): Widget =>
  Widget.create(
    dbWidget.id,
    dbWidget.text,
...
export class JsonWidgetRepository implements WidgetRepository {
  constructor(private db: WidgetDb) {}

  findAll(): Widget[] {
    return this.db.getWidgets().map((dbWidget) => toDomainWidget(dbWidget))
  }
  ...
  findById(id: number): Widget | undefined {
    ...
    return toDomainWidget(dbWidget)
  }
